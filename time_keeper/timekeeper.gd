...
@export var use_day_time_for_shader : bool = true :
	set( value ) :
...
func process_world_tick(time: Vector4i):
	target_time = time[HOUR] + time[PERIOD]*12.0 + time[MINUTE]/60.0
	if time[DAY] > day_of_year: target_time += HOURS_IN_DAY

func align_times(day: int = 0): 
	day_time=target_time
	day_of_year = day

func _process(delta):
	var diff = target_time-day_time
	day_time += lerpf(0, diff, delta)
...
func _update_moon() -> void :
	...
	if is_instance_valid( moon ) :
		...
		# Disabling light under the horizonw
		var moon_direction = moon.to_global( Vector3( 0.0, 0.0, 1.0 )).normalized()
		moon.light_energy = smoothstep( 0.45, 0.55, moon_direction.y ) * moon_base_enegry
