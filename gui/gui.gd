...
func _process(delta):
	if maximap and get_parent().get_active_inhabitant():
		update_minimap_display(delta)
...
func rotate_minimap(amount: float): target_rotation += amount
...
func update_minimap_display(delta: float, markers: bool = true):
	minimap.rotation = lerp(minimap.rotation, target_rotation, 7.5*delta)
	var minimap_boarder_thickness: int = max(minimap_range/30,2)
	var active_inhab: CharacterBody3D = get_parent().get_active_inhabitant()
